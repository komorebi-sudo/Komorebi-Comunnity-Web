      ...
      <div className={panelClasses}>
          ...
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              <div key={item.id} className="flex gap-4 bg-slate-50 p-4 rounded-[1.5rem] relative group border border-slate-100">
                ...
                <div className="flex-1 flex flex-col justify-between">
                  ...
                  <div className="flex items-center justify-between mt-3">
                    ...
                    <div className="flex items-center bg-white rounded-full p-1 shadow-sm border border-slate-100">
                      <button onClick={() => updateQuantity(item.id, item.quantity - 1)} className="w-7 h-7 flex items-center justify-center rounded-full text-slate-500 hover:bg-slate-100 hover:text-slate-800 transition-colors">
                        <Minus size={14} />
                      </button>
                ...
                <button onClick={() => removeFromCart(item.id)} className="absolute top-4 right-4 text-slate-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-all bg-white p-1.5 rounded-full shadow-sm">
                  <Trash2 size={16} />
                </button>
              </div>
            ))
        ...
        {cart.length > 0 && (
          <div className="p-6 border-t border-slate-100 bg-white">
            ...
            <button onClick={handleCheckout} className="w-full bg-slate-800 text-white font-bold py-4 rounded-full shadow-md hover:bg-slate-700 hover:shadow-lg transition-all transform hover:-translate-y-0.5">
              Proceder al Pago
            </button>
          </div>
        )}
