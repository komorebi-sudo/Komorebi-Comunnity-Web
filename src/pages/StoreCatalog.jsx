...
export default function StoreCatalog() {
  ...
  useEffect(() => {
    async function fetchStoreAndProducts() {
      try {
        let { data, error } = await supabase.from('stores').select('*, products(*)').eq('slug', storeSlug).single();
        if (error) throw error;
        setStore(data);
      } catch (err) { console.error("Error al cargar la tienda:", err.message); } 
      finally { setIsLoading(false); }
    }
    if (storeSlug) fetchStoreAndProducts();
  }, [storeSlug]);

  if (isLoading) return <div className="min-h-screen bg-[#faf9f8] flex flex-col items-center justify-center text-slate-500"><Store className="animate-bounce mb-4 text-pink-400" size={32} /><p>Abriendo las puertas de la tienda...</p></div>;
  if (!store) return <div className="min-h-screen bg-[#faf9f8] flex flex-col items-center justify-center text-slate-700"><h1 className="text-2xl font-bold mb-2">Tienda no encontrada</h1><Link to="/" className="bg-pink-100 text-pink-600 px-6 py-2.5 rounded-full font-bold">Volver al Mall</Link></div>;
  ...
  if (activeFilter === 'destacados') processedProducts = processedProducts.filter(p => p.is_trending === true);
  if (activeFilter === 'ofertas') processedProducts = processedProducts.filter(p => p.badge && p.badge.toLowerCase().includes('oferta'));
  if (activeFilter === 'precio-menor') processedProducts.sort((a, b) => a.price - b.price);
  if (activeFilter === 'precio-mayor') processedProducts.sort((a, b) => b.price - a.price);
        ...
        <div className="max-w-6xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            <Link to="/favoritos" className="p-2.5 bg-white rounded-full shadow-sm hover:shadow-md transition-shadow relative text-slate-600">
              <Heart size={20} />{favorites.length > 0 && <span className="absolute -top-1 -right-1 bg-pink-400 text-white text-[10px] font-bold h-5 w-5 rounded-full flex items-center justify-center shadow-sm">{favorites.length}</span>}
            </Link>
            <button onClick={toggleCart} className="p-2.5 bg-white rounded-full shadow-sm hover:shadow-md transition-shadow relative text-slate-600">
              <ShoppingBag size={20} />{cartCount > 0 && <span className="absolute -top-1 -right-1 bg-pink-400 text-white text-[10px] font-bold h-5 w-5 rounded-full flex items-center justify-center shadow-sm">{cartCount}</span>}
            </button>
            <Link to="/perfil" className="hidden sm:flex items-center space-x-2 bg-slate-800 text-white px-5 py-2.5 rounded-full text-sm font-semibold hover:bg-slate-700 transition-colors shadow-sm"><User size={16} /><span>Mi Perfil</span></Link>
          </div>
        </div>
      </header>

      <div className={`w-full h-40 bg-slate-100 border-b border-slate-100 relative overflow-hidden`}>
        {store.banner_url ? (
          <img src={store.banner_url} alt={`Banner de ${store.name}`} className="absolute inset-0 w-full h-full object-cover z-0" />
        ) : (
        ...
        <div className="max-w-6xl mx-auto px-6 h-full flex items-end pb-6 relative z-10">
           <Link to="/" className="absolute top-6 left-6 flex items-center bg-white/80 backdrop-blur-md px-3 py-1.5 rounded-full text-xs font-semibold hover:bg-white transition-colors text-slate-700 shadow-sm"><ArrowLeft size={14} className="mr-1" /> Volver</Link>
        </div>
      </div>
        ...
        <section className="flex-1 mt-4 md:mt-12">
          {store.products && store.products.length > 0 && (
            <div className="flex flex-col sm:flex-row gap-4 mb-8">
              <div className="flex-1 relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} /><input type="text" placeholder={`Buscar en ${store.name}...`} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-11 pr-4 py-3 rounded-full bg-white border border-slate-100 focus:outline-none focus:border-pink-200 transition-all text-sm font-medium shadow-sm" /></div>
