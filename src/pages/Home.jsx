...
import { Search, ShoppingBag, User, Menu, Store, TrendingUp, ArrowRight, Star, Gamepad2, BookOpen, Shirt, Sparkles, Heart } from 'lucide-react';
        ...
        <header className="bg-[#faf9f8]/80 backdrop-blur-xl sticky top-0 z-50 border-b border-slate-100">
          <div className="max-w-6xl mx-auto px-6 h-20 flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              <button className="p-2.5 bg-white rounded-full shadow-sm hover:shadow-md transition-shadow relative text-slate-600">
                <ShoppingBag size={20} />
                {cartCount > 0 && (
        ...
        <main className="max-w-6xl mx-auto px-6 pb-24">
          ...
          <section className="mb-24">
            <div className="flex items-center justify-between mb-10">
              ...
              <button className="text-sm font-semibold text-pink-500 hover:text-pink-600 flex items-center">
                  ...
                  <div className="pt-10 pb-6 px-6 flex flex-col flex-1 bg-white relative z-10">
                    ...
                    <div className="mt-auto flex items-center justify-between">
                      <span className="flex items-center text-sm font-semibold text-slate-600">
                        <Star size={16} className="text-yellow-400 fill-yellow-400 mr-1.5" /> {store.rating}
                      </span>
                      {/* AQUI ESTA EL CAMBIO A LINK */}
                      <Link 
                        to={`/tienda/${store.id}`} 
                        className="bg-slate-50 text-slate-700 px-4 py-2 rounded-full text-sm font-semibold group-hover:bg-pink-50 group-hover:text-pink-600 transition-colors"
                      >
                        Visitar
                      </Link>
                    </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
              {mockTrending.map(product => (
                <div key={product.id} className="bg-white rounded-[2rem] p-5 flex flex-col relative group shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100">
                  ...
                  <div className={`${product.bgColor} h-48 rounded-[1.5rem] flex items-center justify-center text-7xl mb-5 transition-transform duration-500 group-hover:scale-95`}>
                    {product.icon}
                  </div>
                  ...
                  <h4 className="font-semibold text-slate-800 leading-snug mb-4 flex-1">
                    {product.name}
                  </h4>
                  ...
                  <div className="flex items-center justify-between mt-auto">
                    ...
                    <button className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-600 hover:bg-slate-800 hover:text-white transition-colors">
                      <ShoppingBag size={18} />
                    </button>
                  </div>
                </div>
          ...
          {/* CTA */}
          <section className="bg-gradient-to-br from-pink-100 to-purple-100 rounded-[3rem] p-10 md:p-16 text-center relative overflow-hidden border border-white/50 shadow-sm">
            <div className="relative z-10 max-w-xl mx-auto">
              <h2 className="text-3xl md:text-4xl font-bold mb-4 text-slate-800">
                ¿Creas cosas geniales?
              </h2>
              <p className="text-lg mb-8 text-slate-600">
                Únete a nuestra comunidad de vendedores y abre tu propia tienda en minutos. Es súper fácil.
              </p>
              <button className="bg-white text-slate-800 px-8 py-4 rounded-full font-bold shadow-sm hover:shadow-md hover:scale-105 transition-all">
                Abrir mi tienda
              </button>
            </div>
          </section>
